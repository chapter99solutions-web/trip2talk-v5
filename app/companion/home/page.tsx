...
import { StaffHomeRouter } from '@/components/companion/staff/StaffHomes';
...
import { isGuestSession } from '@/lib/companion/staff-session';
...
export default function CompanionHomePage() {
  ...
  if (!isGuestSession(session)) {
    return <StaffHomeRouter session={session} />;
  }

  if (!session.trip || !session.booking) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white/60 px-6 text-center">
        ไม่พบข้อมูลทริป กรุณาเข้าสู่ระบบใหม่
      </div>
    );
  }
