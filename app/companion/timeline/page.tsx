...
import CohostTimelineOverview from '@/components/companion/staff/CohostTimelineOverview';
...
export default function CompanionTimelinePage() {
  ...
  if (sessionStaffRole(session) === 'cohost') {
    return <CohostTimelineOverview />;
  }

  const trip = session.trip;
  if (!trip) {
    return <p className="p-6 text-white/60 text-center">ไม่พบทริป</p>;
  }
